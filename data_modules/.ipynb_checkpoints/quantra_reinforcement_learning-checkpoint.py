...
def time_now():
    ...
    #print("date and time =", now.strftime("%H:%M:%S %d.%m.%Y"))
...
class ExperienceReplay(object):
    ...
    def process(self, modelQ, modelR, batch_size=10):
        ...
        for i, idx in enumerate(np.random.randint(0, len_memory, size=inputs.shape[0])):
            ...
            targets[i] = modelR.predict(state_t, verbose=0)[0]

            """---Calculate the reward at time t+1 for action at time t---"""
            Q_sa = np.max(modelQ.predict(state_tp1, verbose=0)[0])
    ...
    while True:
        ...
        while not game_over:
            ...
            if np.random.rand() <= epsilon:
                ...
                q = q_network.predict(state_t, verbose=0)
        ...
        #time_now()
        print("r_time:" + time_now() + " | Trade:{:03d} | pos:{} | len:{} | approx_cum_ret:{:,.2f}% | trade_ret:{:,.2f}% | eps:{:,.4f} | t_time:{} | idx:{}".format(
